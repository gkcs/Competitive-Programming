...
public class Simulator {
    ...
    private void playGame() {
        ...
        for (int moveNumber = 0;
             moveNumber < 10 && !board.isTerminated(player, 0, moveNumber);
             moveNumber++, player = player == 1 ? 2 : 1) {
            MinMax minMax = new MinMax(moveNumber);
            minMax.setTest(true);
            Move move = minMax.iterativeSearchForBestMove(player, board);
            System.out.println(toString(board.board) + "new Move(BOARD.CELLS["
                                       + move.start.x + "][" + move.start.y + "]" +
                                       ",BOARD.CELLS[" + move.end.x + "][" + move.end.y +
                                       "]" + ", " + move.player + ", " + move.isAJump + "));");
